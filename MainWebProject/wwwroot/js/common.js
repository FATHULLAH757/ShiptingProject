...
ilms.routers = {
    // Controller Routes
    InsertWorkOrder: "/Home/WorkOrder",
    InsertDriver: "/Home/CreateDriver"
};
...
ilms.partialPageLoadingPost = function (pageUrl, dataObj) {
    ...
    $.ajax({
        ...
        success: function (data, status, xhr) {
            if (xhr.getResponseHeader('LOGIN_SCREEN_Ready') === '1') {
                // You can optionally handle the session expiration condition here
                // without calling ilms.WhenSessionIsExpired()
                return;
            }
            $('#RenderBody').empty().html(data);
            ilms.hideLoading();
